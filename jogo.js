...
let cobra = [];
let direcao = { x: 1, y: 0}; //Começa indo pra direita
...
function renderizar() {
    ...
    for (let i = 1; i < cobra.length; i++) {
        celulas[cobra[i].y][cobra[i].x].clasList.add ("cobra");
...
function tick() {
    ...
    if(novaX < 0 || nova >= COLUNAS || novaY < 0 || novaY >= LINHAS) {
        encerrarJogo();
        return;
    }
...
function esconderOverlay() {
    document.getElementById("overlay").classList.remove("visível");
}
